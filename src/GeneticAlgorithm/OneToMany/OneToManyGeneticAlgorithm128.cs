namespace GeneticAlgorithm.OneToMany
{
    public class OneToManyGeneticAlgorithm128
    {
        ...
        private Individual128[] _currentGeneration;
        private Individual128[] _nextGeneration;

        private readonly List<decimal> _bestSolution;
        private decimal _bestFitness;

        public OneToManyGeneticAlgorithm128(OneToManyGeneticOptions options)
        {
            ...
            _currentGeneration = new Individual128[_generationSize];
            for (var i = 0; i < _generationSize; i++)
            {
                _currentGeneration[i] = new Individual128();
            }

            _nextGeneration = new Individual128[_generationSize];
            for (var i = 0; i < _generationSize; i++)
            {
                _nextGeneration[i] = new Individual128();
            }
        ...
        private void Crossover()
        {
            for (var i = 0; i < _generationSize; i += 2)
            {
                ...
                var (mask1, mask2) = Masks[countPart1];
                child1.BitMask = parent1.BitMask & mask1 | parent2.BitMask & mask2;
                child2.BitMask = parent2.BitMask & mask1 | parent1.BitMask & mask2;
        ...
        private void FillGeneration(Individual128[] generation, OneToManyInitialSeeding filling)
        {
            switch (filling)
            {
        ...
        private void UpdateFitness(Individual128 individual)
        {
            individual.Fitness = CalculateFitness(individual, out var countSet);
            individual.BitsSet = countSet;
        }

        private decimal CalculateFitness(Individual128 individual, out int bitsSet)
        {
            var sum = _subsetSum;
            bitsSet = 0;
